...
VECTOR getinitrot(VECTOR p1,VECTOR p2)
{
  ...
  FLOAT d1 = sqrt(p1[1]*p1[1] + p1[2]*p1[2]);
  FLOAT d2 = sqrt(p2[1]*p2[1] + p2[2]*p2[2]);
...
void clear_vector(FLOAT *v,int nl,int nh)
{
  int i;
...

{
  ...
  v=(FLOAT *)myalloc((unsigned) (nh-nl+1)*sizeof(FLOAT));
...
void add_vector(FLOAT *to,int l,int h, FLOAT *from,int fl,int fh)
{
  int i,j;
...
void subtract_vector(FLOAT *to,int l,int h, FLOAT *from,int fl,int fh)
{
  int i,j;
...
void copy_vector(FLOAT *to,int l,int h, FLOAT *from,int fl,int fh)
{
  int i,j;
...
FLOAT *clone_vector(FLOAT *v,int nl,int nh,int newl,int newh)
{
  FLOAT *nv;
  nv = vector(newl,newh);
...
FLOAT *subvector(FLOAT *v,int oldl,int oldh,int newl)
{
  FLOAT *m;
  m = &(v[oldl])-newl;
  return(m);
...
FLOAT *cross_vector(FLOAT *a,FLOAT *b)
{
  FLOAT *c=vector(1,3);
...
FLOAT dot_vector(int n,FLOAT *a,FLOAT *b)
{
  int i;
  FLOAT d=0.0;
...
FLOAT vector_length(int n,FLOAT *a)
{
  FLOAT d = dot_vector(n,a,a);
  return(sqrt(d));
}

void multvector(int n,FLOAT *a, FLOAT p)
{
  int i;
  for(i=1;i<=n;i++)
  {
...
FLOAT *unit_vector(int n,FLOAT *a)
{
  FLOAT *u = clone_vector(a,1,n,1,n);
...

{
 ...
 //MATRIX *m;
  ...
  mh->real_mat = (MATRIX  )myalloc( (unsigned)((nrh-nrl+1)*sizeof(FLOAT *)));
  if(mh->real_mat == NULL) 
  {
...
MATRIX *matrix(int nrl,int nrh,int ncl,int nch )
{
  ...
  for(i=nrl;i<=nrh;i++)
  {
    m[i] = (FLOAT *)myalloc((unsigned) (nch-ncl+1)*sizeof(FLOAT));
...
int add_matrix_rows( MATRIX *mp, int nr)
{
   ...
   if(nnr > 0 )
   {
     ...
     if(nnr < 100)nnr = 100;
     mh->real_mat = (MATRIX  )myalloc( (unsigned)((nnr+mh->maxp)*sizeof(FLOAT *)));
...
void normalize_matrix(FLOAT *ax,FLOAT *ay,FLOAT *az)
{
  FLOAT *ux,*uy,*uz;   

  FLOAT a = vector_length(3,az);
/* printf("length = %e,%e\n",a,b);*/
...
FLOAT dot(FLOAT *v1,int l1,int h1, FLOAT *v2,int l2,int h2)
{
  int i,j;
  FLOAT d=0.0;
...
FLOAT *matrix_vector(MATRIX *m,int rl,int rh,int cl,int ch,
                     FLOAT *v,int l,int h)
{
  FLOAT *r;
  int i,j;
...
FLOAT *solve(MATRIX *a,FLOAT *L, int n)
{
    int i;
    FLOAT d;
    int *indx;
...
/*
typedef FLOAT PNT_A[2];
typedef FLOAT PNT3_A[3];
typedef PNT_A *PNT;
typedef PNT3_A *PNT3;
*/
...
FLOAT *lsq(MATRIX *A,FLOAT*L,int nu,int ne)
{
    int i;
    MATRIX *AT,*ATA;
    FLOAT *ATL,*R;
...
//printf(" 1 anorm,xx %e,  %e,  %e\n",anorm,rv1[l],(FLOAT)(fabs(rv1[l])+anorm));
...
// printf(" 2 anorm,xx %e,  %e,  %e\n",anorm,w[nm],(FLOAT)(fabs(w[nm])+anorm));
...
void svdfit(PNTLST A, VECTOR y, VECTOR sig, int ndata, VECTOR a, int ma,
            PNTLST u, PNTLST v, VECTOR w, FLOAT *chisq,
            void (*funcs)(FLOAT,PNTLST, int)  )
...
void mrqcof(VECTOR x, VECTOR y, VECTOR sig,int ndata, VECTOR a, int ia[],
            int ma, PNTLST alpha, VECTOR beta, FLOAT *chisq ,
            void (*funcs)(VECTOR, VECTOR, FLOAT *, VECTOR ,int))
...
void mrqmin(VECTOR x, VECTOR y, VECTOR sig, int ndata, VECTOR a, int ia[],
            int ma, PNTLST covar, PNTLST alpha, FLOAT *chisq,
            void (*funcs)(FLOAT, VECTOR,FLOAT *,VECTOR, int), FLOAT *alamda)
...
void BBlsq(PNTLST ATAx, PNTLST P, PNTLST Q, PNTLST S,VECTOR AWT, VECTOR s, VECTOR RR,
     int nu,int ne,int nf,int np, VECTOR photosum, VECTOR pntsum, PNTLST *fp,int nff,int npf, FLOAT *chisq)
{
...
void pnt2film(FLOAT fl, VECTOR tm, VECTOR pnt, FLOAT* x, FLOAT* y)
{
  /*******
...
*******/
  ...
  FLOAT px = tm[1]*pnt[1] + tm[2]*pnt[2] + tm[3]*pnt[3] + tm[10];
  FLOAT py = tm[4]*pnt[1] + tm[5]*pnt[2] + tm[6]*pnt[3] + tm[11];
  FLOAT pz = tm[7]*pnt[1] + tm[8]*pnt[2] + tm[9]*pnt[3] + tm[12];
...
void pnt2film_error(FLOAT fl, VECTOR tm, VECTOR pnt,VECTOR pp, FLOAT* dx, FLOAT* dy)
{

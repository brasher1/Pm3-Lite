...
int make_cone_by_lines(int nl,int *npl, PNTLST *pnts,PNTLST tms,
                        PNTLST sep, PNTLST setm , struct Cone &snout,
                        struct _transform &trans)

{
  ...
  PNTLST cone = cone_by_edge( nl,npl,pnts,tms);
...
//printf_vector((*setm)[1],1,12);
  ...
  PNTLST start = intersect_line_cone( k,(*cone)[1],(*cone)[2],&((*setm)[0][9]),(*sep)[0]);
  PNTLST end   = intersect_line_cone( k,(*cone)[1],(*cone)[2],&((*setm)[1][9]),(*sep)[1]);

  subtract_vector( (*start)[1],1,3,(*cone)[1],1,3);
  subtract_vector( (*end)[1],1,3,(*cone)[1],1,3);
 
 PHOTO_FLOAT sd = dot_vector( 3, (*start)[1], (*cone)[2] );
 PHOTO_FLOAT ed = dot_vector( 3, (*end)[1], (*cone)[2] );
 ...
 snout.bottomRadius = k*sd;
 snout.topRadius = k*ed;
 snout.xOffset = 0.0;
 snout.yOffset = 0.0;
 snout.height = sd-ed;
  ...
  trans.pozition.pos_x = (*cone)[1][1] + 0.5*(sd+ed)*( (*cone)[2][1]);
  trans.pozition.pos_y = (*cone)[1][2] + 0.5*(sd+ed)*( (*cone)[2][2]) ;
  trans.pozition.pos_z = (*cone)[1][3] + 0.5*(sd+ed)*( (*cone)[2][3]);
...
/***** Start/End Points based on surface intersection *****/  
   ...
   make_cyl_by_endpnts(R[1],(*sep)[0],(*sep)[1],cyl,trans);
